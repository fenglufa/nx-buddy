...
- 审图长任务（续）：证据抽取目前覆盖建模件 Part 段；工程图段（title_block/sheet/layers/weld/parts_list/thickness/hole_pattern 的 `inspect_*` 抽取器）按 §9.2 批次接入后，同一状态机自动补齐 FRAME/TITLE/LAYER/VIEW 规则判定；报告"图号"暂取文件名，标题栏批次后改读真实图号。
