...
装在哪个盘无关）或部署脚本失败，会弹错误提示说清原因并给出手动补部署命令；成功不再弹窗打断，
部署明细看 deploy_plugin.ps1 的输出窗口。装完可选立即启动托盘，并在 `HKCU\...\Run` 登记自启。
...
2. **托盘主页**：托盘启动（开机自启、桌面快捷方式、安装完立即启动）会先自动打开主页；
   之后**左键单击**托盘图标再次打开（右键仍是快捷菜单）。托盘单实例——重复双击桌面图标
   不会再起一套，只会把已在跑的实例的主页唤到前台。三个分页：
   "状态与操作"（授权/NX 连接/MCP/审图四态 + 复制 MCP 配置 + 打开日志与报告目录 + 路径设置 +
   **开机自动启动开关**）、
   "审图工作台"、"规则管理"。主页右上角关闭=收回托盘，审图任务继续跑；"退出"才终止托盘
   （并带走它自己的宿主进程）。
   自启开关直接读写 `HKCU\...\Run\NXAssistant` 值（用户级、免管理员；勾选开启时写入当前运行的
   托盘路径）。注意：重新运行安装程序会按安装器策略重写该值（即恢复"开"），关过的升级后需在主页再勾掉。
...
1. **"选择图纸目录…"任选一个含 `.prt` 的文件夹（可含子目录，任意位置均可）**：目录在工作区
   （默认 `%LOCALAPPDATA%\NXAssistant\workspace`）内就直接审；在工作区外，托盘会自动把 `.prt`
   复制成工作区副本再审（FILE-001 沙箱不让宿主程序随意访问任意路径；原件全程只读不动，
   状态行会显示副本位置）；
2. 点 **发起审图**，立刻拿到 run_id，后台逐张判定不卡界面；
3. 进度每 2 秒自动刷新（状态/已完成张数/当前文件）；"取消"在当前这张完成后生效，
   报告只含已完成部分；中断/失败后可选中该 run **续跑**（自动沿用该 run 登记的目录，含导入副本；从下一张接着审）；
...
主页 →"规则管理"分页：表格列出规则包全部规则（编号/组/名称/级别），**双击行**看规则详情
（当前生效的数据文件与用户改动状态），勾选即启用/禁用，
"改参数"编辑规则引用的标准系列数据（如孔系列、圆角系列；无独立数据的规则会说明原因），
保存写入用户态覆盖层并留变更日志
（谁在何时禁了哪条），宿主下一次守卫/审图即生效；"重置"清空全部覆盖回到基线。
**禁用 ≠ 删除**（等效停用，随时勾回）；规则的新增/删除不支持 GUI 编辑——判定逻辑内建在引擎里，
升级规则包后新规则默认启用；整包不可用仍 fail-closed。
