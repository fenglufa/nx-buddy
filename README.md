...
pwsh build/build.ps1            # 发布到 dist/（不写 %UGII_USER_DIR%，避免覆盖 NX-MCP）
...
- 授权批次：离线验签 + 一 Key 一机 + `LICENSE_INVALID` 闸门；`licensing/` 签发工具。
- 读/写工具批次：移植 §9.1 各 `_op_` 到 C#（含写后回读护栏、stable_id、表达式字符串通道）。
- 规则引擎：读 company_v3 JSON，画图为防 / 审图为查同一套。
- 审图长任务：`review_folder`/`review_status`/`review_findings`（run_id 状态机 + xlsx 报告）。
...
- 部署插件进 `%UGII_USER_DIR%\startup` 时**合并、不覆盖** NX-MCP 既有 DLL；这一步需重启 NX，改动前确认。
