...
| 第十四片续8 | 安装尾声成功不弹窗，仅部署失败/缺 UGII_USER_DIR 弹错 | c14d971 |
| 第十四片续9 | 主页"开机自动启动"开关（读写 HKCU Run，安装器同名值） | 8e6b1e5 |
...
**审图目录体验（同日第十四片续5）**：用户看不懂"审图目录（须在 workspace 根内）"，要求直接审实际图纸目录。
FILE-001 沙箱保留在宿主侧不动（Agent 共用同一 `review_folder`，不能给人机对话开后门），改由托盘兜底：
"选择图纸目录…"可任选位置，目录在工作区外时**自动把 `*.prt`（含子目录结构）复制到
`workspace\审图导入\<时间戳>\` 再审副本**，原件零接触，状态行播报副本位置；文案全部白话化
（主页状态行同步）。顺带发现并修掉一个**真 bug**：续跑要求 path 与 run 登记一致（宿主
`ReviewOrchestrator` 强校验），而输入框可能已被用户改动/选的是外部目录——续跑改为不再读输入框，
直接取 `run.json` 的 `path` 字段；副本被清理时给出可操作报错。`smoke_tray` T1–T6 全绿；
新包 5,433,781B / SHA1 9c1502400e34e7e0e8c6c3c8a6dfb27cece97ad6。
若想彻底不复制（图纸量大时省磁盘）：主页"设置"里把工作区根直接指到真实图纸目录即可（热生效）。

**升级安装"装了等于没装"修复（同日第六轮装机反馈，实机根因定位）**：用户报告不卸载直接装，
装多少次都不是新包；卸载后重装才正常。在开发机真实安装目录做"篡改-升级"对照实验
（改已装 DLL 时间戳 + 给无版本资源的 install.ps1 追加标记 → 静默升级 → 验回滚），日志给出铁证：
`Version of our file: 1.0.0.0 / Version of existing file: 1.0.0.0 / Same version. Skipping.`
——Inno 默认策略是"两边都有版本号且相等就不覆盖"，而 .NET 发布产物全带 1.0.0.0 文件版本且我们
不逐版递增，于是升级时**每个 DLL/exe 全部跳过**（install.ps1 没版本号资源反而被照常复制，
症状完全吻合）。修复=[Files] 全部条目加 `ignoreversion`，升级无条件覆盖自家产物；
复测 0 个 Skipping、时间戳全恢复、装机与 dist 哈希逐一相等。新包 5,490,022B /
SHA1 27911d2cf13138b7b14679e7e7c4c7fe2758c00a。教训：**闭源产品不 bump 程序集版本时，
Inno 默认行为=升级静默失效，ignoreversion 是必选项而非优化项**；此类"装了没生效"必须用
篡改-重装对照实验在真安装目录验证，ISCC 编译通过说明不了任何事。
附带发现（已随第十四片续7修复）：测试期间起了第二个托盘实例暴露**托盘无单实例互斥**——双击两次桌面图标
会跑两套托盘+宿主。

**托盘单实例（同日第十四片续7）**：`Program.Main` GUI 路径加命名 Mutex `Local\NXAssistant_Tray_SingleInstance`
——抢不到锁的第二实例向 `Local\NXAssistant_Tray_ShowMain` 命名事件发信号后静默退出；第一实例用 300ms UI
定时器轮询 `WaitOne(0)` 收到即弹主页（复用消息循环，免跨线程编组）。`--status-json/--review-probe/--ui-probe`
...
**主页开机自启开关（第十四片续9）**：用户反馈装完默认开机自启、没地方关。新增 `AutoStart`（读写
`HKCU\...\Run\NXAssistant`，与安装器同名值，用户级免管理员）+ 主页"状态与操作"行的
"开机自动启动"勾选框：进入主页时按注册表真值初始化，勾选变更即时落盘，写失败回滚勾选并给出
手动改注册表的指引。开启写的是**当前运行 exe 的路径**（换目录/升级后勾一次即纠正）。
已知边界：重跑安装程序会按 [Registry] 策略把该值重写回"开"（Inno 无条件写），关掉的用户装完需再勾一次——README 已写明。
验证：smoke_tray T6（--ui-probe 走勾选框初始化路径）PASS；真机升级装机（哈希 7ff07301=dist）后
安装版 `--ui-probe` ok、自启值在位、托盘已重拉。新包 5,434,804B / SHA1 2970782eb06cc12006765df2f4a85f093a5a6f41。
...
- **第十五片候选：声明式规则原语（2026-09-22 与用户讨论定向前因）**——"客户每加一条标准都要找厂商发版"
  在商业上不合理。三条路对比：A=声明式条件引擎（规则 JSON 写成"筛选证据子集 → 字段 vs 阈值/系列比对"，
  引擎内置通用评估器，现有 28 条几乎全部可归约；不开安全口子，代价是表达力上限+需 pack schema 校验器，
  非法条件装载即 fail-closed 并报明确错，杜绝"加了永不触发"假象）；B=内嵌脚本引擎（Jint 沙箱，表达力全开
  但安全/调试/验收责任外移，V1 不碰）；C=维持"新逻辑随版本"并商务化为定制服务。
  决定：A 立项为第十五片候选——先盘点 28 条归纳原语清单（约 1-2 天），做通用评估器 + 校验器，
  规则管理页文案随改"支持声明式新增"；复杂超集需求漏斗到 C 报价定制。
