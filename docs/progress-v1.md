...
| 第十四片续6 | 升级安装 ignoreversion 修复（Inno 同版本默认跳过致升级静默失效） | 待回填 |
...
附带发现（记待办）：测试期间起了第二个托盘实例暴露**托盘无单实例互斥**——双击两次桌面图标
会跑两套托盘+宿主。
...
- 托盘单实例（第六轮验收时发现）：现在可以双开托盘（各自主页+各自宿主，两个托盘图标）。
  应加命名 Mutex：第二实例检测到即退出（或唤起第一实例主页）。
